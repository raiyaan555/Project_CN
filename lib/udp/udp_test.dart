import 'dart:convert';
import 'dart:io';
import 'package:cn_test_app/udp/udp_socket_base.dart';
...
class UDPTest {
  ...
  send(String ms) async {
    ...
    if (cdsSocket != null) {
      ...
      final datagram = await cdsSocket.receive(timeout: 3);
  ...
  bindMulticastServer(String ip, int port) async {
    ...
    while (true) {
      final datagram = await socketServer?.receive(timeout: 3);
      if (receiveEvents.containsKey(RESPONSE))
        for (var f in receiveEvents[RESPONSE]) {
          f(datagram);
        }
